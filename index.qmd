...
The metric attains a value of 1 indicates that all checks are independent: $H(X_1, X_2, \cdots, X_k)=\sum_{i= 1}^k H(X_i)$ and provide unique information to diagnose the unexpected outcome (trivia case when there is only one check), and a value of 0 when the checks are identical: $H(X_1, X_2, \cdots, X_k)- \sum_{i= 1}^k H(X_i) = H(X_i)/ k H(X_i) = 1/k$. 
