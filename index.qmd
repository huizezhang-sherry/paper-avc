...
-   check 1: the check fails if the mean step count is below 8,200
-   check 2: the check fails if the 30th percentile of the observed step counts is below 8,200
-   check 3: the check fails if the standard deviation of the observed step counts exceeds 2,500.
...
Returning to the step count example introduced in @sec-toy, the logic regression model is fitted to the three analysis checks described previously to generate the prediction of the unexpected outcome, which is whether the number of days with fewer than 8,000 steps is greater than 5. The predictions from the logic regression model can be compared with the simulated true outcome for calculating the precision and recall metrics. @fig-logic-reg shows the best-fitting logic regression model, which is a combination of check 1 ("mean step count is below 8,200") and check 3 ("standard deviation exceeds 2,500") with an OR operator. In other words, if either check 1 or check 3 is true, then we would predict an unexpected outcome in the analysis (i.e. the number of days with fewer than 8,000 steps is greater than 5).

```{r}
#| label: fig-logic-reg
#| fig.width: 6
#| fig.heigh: 2
#| fig-cap: Logic regression model fitted to the three unit tests (check 1, check 2, check 3). The model suggests using an OR rule to combine check 1 and check 3 to predict the outcome expectation.
...
@tbl-logic-reg presents the calculated precision, recall, and independence for the three individual checks and the combined check rule (check 1 OR check 3) from the logic regression. We also include the metric calculated from fitting a regression tree model to the data to compare the performance of the logic regression model. The harmonic and arithmetic means are included to combine the three measures. The results show that the two checks produced by the logic regression can accurately predict 83.6% cases of all *actual unexpected results* in the simulation data. Furthermore, 82.1% of all *predicted unexpected results* were in fact observed to be unexpected.  
...
tbl <- tibble(check = c("check 1: sample size <= 200", "check 2: cor(m, PM10) < -0.03",
                        ...
                        "(not check 1 AND check 2) AND (check 3 OR check 4)")) |> bind_cols(df |> dplyr::select(-id, -overlapping)) |> rename(`indep.` = independence) 
