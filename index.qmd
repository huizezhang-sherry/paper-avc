...
[@peng_diagnosing_2021]

TODO: what if the expectation is "wrong"

## Data analysis checks in statistical software
  
  In data analysis, checks are often performed to ensure the quality of the data and the validity of the model. For data checks,  
  
  * The `assertr` package [@assertr] provides a set of functions to check the quality of data, including the presence of missing values, the distribution of variables, and the relationship between variables. 
  * Similarly, the `pointblank` package [@pointblank] is a data validation tool that allows users to define data quality checks. 
  
  For model checks, 
  
  * the `performance` package [@performance] provides a set of functions to assess the performance of a model, including residual analysis, goodness-of-fit tests. 
  * The `yardstick` package [@yardstick] provides the calculation of model performance metrics in the `tidymodels` [@tidymodels] ecosystem. 

These checks test on specific aspect of the data or the model but doesn't reflect the underlying assumption of the data analysts about the data or the problem context. It may be obvious to see the assumption if someone is testing for data quality on whether a column in the data frame is between two values. In a much complex analysis, it is difficult to trace the assumptions made by the data analysts. Currently, most of these assumptions are not documented or discussed in the analysis process, or at least in a machine readable format. 
