...
\item
  check 2: the check fails if the 30th percentile of the observed step
  counts is below 8,200
\item
  check 3: the check fails if the standard deviation of the observed
  step counts exceeds 2,500.
\end{itemize}
...
best-fitting logic regression model, which is a combination of check 1
(``mean step count is below 8,200'') and check 3 (``standard deviation
exceeds 2,500'') with an OR operator. In other words, if either check 1
or check 3 is true, then we would predict an unexpected outcome in the
...
unit tests (check 1, check 2, check 3). The model suggests using an OR
rule to combine check 1 and check 3 to predict the outcome expectation.}

\end{figure}%

Table~\ref{tbl-logic-reg} presents the calculated precision, recall, and
independence for the three individual checks and the combined check rule
(check 1 OR check 3) from the logic regression. We also include the
metric calculated from fitting a regression tree model to the data to
compare the performance of the logic regression model. The harmonic and
arithmetic means are included to combine the three measures. The results
show that the two checks produced by the logic regression can accurately
...
\begin{longtable}[]{@{}
  ...
  >{\raggedleft\arraybackslash}p{(\columnwidth - 12\tabcolsep) * \real{0.1688}}
...
parsimony (independence) metrics for each individual check and for the
combined check rule (check 1 OR check 3) derived from the logic
regression model. The harmonic and arithmetic means of the three metrics
are included to evaluate the quality of the checks in diagnosing
...
\bottomrule\noalign{}
\endlastfoot
1 & 0.482 & 0.964 & 1.000 & 0.730 & 0.815 & check 1 \\
2 & 0.214 & 1.000 & 1.000 & 0.450 & 0.738 & check 2 \\
3 & 0.589 & 0.805 & 1.000 & 0.762 & 0.798 & check 3 \\
check 1 OR check 3 & 0.821 & 0.836 & 0.999 & 0.879 & 0.886 & NA \\
regression tree & 0.821 & 0.836 & 1.000 & 0.879 & 0.886 & NA \\

\end{longtable}

For comparison, the regression tree produces a similar prediction to the
logic regression, by first splitting on check 1 and then splitting on
check 3, and results in the same accuracy and overall score as the logic
...
\begin{longtable}[]{@{}
  >{\raggedright\arraybackslash}p{(\columnwidth - 10\tabcolsep) * \real{0.5368}}
...
(not check 1 AND check 2) AND (check 3 OR check 4) & 0.760 & 0.880 & 1 &
0.869 & 0.880 \\
